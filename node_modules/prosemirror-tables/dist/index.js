...
function cellNear($pos) {
  for (var after$1 = $pos.nodeAfter, pos = $pos.pos; after$1; after$1 = after$1.firstChild, pos++) {
    var role = after$1.type.spec.tableRole;
    if (role == "cell" || role == "header_cell") { return $pos.doc.resolve(pos) }
  }
  for (var before = $pos.nodeBefore, pos$1 = $pos.pos; before; before = before.lastChild, pos$1--) {
    var role$1 = after.type.spec.tableRole;
...
var CellSelection = (function (Selection$$1) {
  ...
  CellSelection.prototype.content = function content () {
    ...
    return new prosemirrorModel.Slice(prosemirrorModel.Fragment.from(rows), 1, 1)
  };
...
function arrow(axis, dir) {
  return function (state, dispatch, view) {
    ...
    if (sel instanceof CellSelection) {
      dispatch(state.tr.setSelection(prosemirrorState.Selection.near(sel.$headCell, dir)));
      return true
    }
    ...
    if (axis == "horiz") {
      dispatch(state.tr.setSelection(prosemirrorState.Selection.near(state.doc.resolve(sel.head + dir), dir)));
      return true
    } else {
    ...
    state: {
      init: function init(_, state) {
        this.spec.props.nodeViews[tableNodeTypes(state.schema).table.name] =
          function (node) { return new TableView(node, cellMinWidth); };
...
ResizeState.prototype.apply = function apply (tr) {
  ...
  if (state.activeHandle > -1 && tr.docChanged) {
    var $handle = tr.doc.resolve(tr.mapping.map(state.activeHandle, -1));
